...
export default function HelmetHall() {
  ...
  return (
    <section
      id="helmets"
      className="relative min-h-screen bg-vk-black overflow-hidden py-24 flex flex-col justify-center"
    >
      ...
      <div className="relative z-10 max-w-[100vw] mx-auto px-4">
        ...
        >
          <h2 className="text-4xl md:text-6xl lg:text-7xl font-black uppercase tracking-tight">
            ...
            <br />
            <span className="text-vk-red font-serif">HALL OF FAME</span>
          </h2>
        </motion.div>
            ...
            {helmets.map((helmet, index) => {
              ...
              return (
                ...
                >
                  ...
                  {isActive && (
                    <div className="absolute -inset-20 bg-vk-red/10 rounded-full blur-3xl -z-10 animate-pulse" />
                  )}
        ...
        <div className="relative h-32 mt-8 flex justify-center items-center overflow-hidden">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <p className="text-xl md:text-2xl text-vk-red font-serif mt-2">{helmets[activeIndex].year}</p>
            </motion.div>
          </AnimatePresence>
        </div>
        ...
        <div className="flex justify-center gap-8 mt-8">
          <button
            ...
            className="p-4 rounded-full border border-white/20 hover:bg-white/10 hover:border-vk-red transition-all disabled:opacity-30 disabled:cursor-not-allowed group"
          >
            <ChevronLeft className="w-6 h-6 text-white group-hover:text-vk-red" />
          </button>
            ...
            {helmets.map((_, idx) => (
              <button
                ...
                className={`w-2 h-2 rounded-full transition-all duration-300 ${
                  idx === activeIndex ? "w-8 bg-vk-red" : "bg-white/20 hover:bg-white/50"
                }`}
              />
            ))}
          </div>
          <button
            ...
            className="p-4 rounded-full border border-white/20 hover:bg-white/10 hover:border-vk-red transition-all disabled:opacity-30 disabled:cursor-not-allowed group"
          >
            <ChevronRight className="w-6 h-6 text-white group-hover:text-vk-red" />
          </button>
        </div>
      </div>
    </section>
